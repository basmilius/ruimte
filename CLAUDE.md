...
- A project is `<folder>/.ruimte/project.json`, version 2: a name, a color, an icon and a list of views with a monotonic `rev` (`apps/server/src/projects`). A view is a canvas (nodes, texts, edges, layouts) or, later, a standalone chat, terminal or browser. The daemon reads version 1 and 2 and writes only 2 (`migrateDocument` in `packages/contracts`); at parse time it refuses a project where a view id or a node id repeats and drops an edge that points outside its own view. Machine state (which view was open, the camera and focus per view, the panels) lives in `<projectId>.local.json` and never in the shared file. The client saves through `project.save` with the rev it loaded and reacts to `project.changed` from the daemon's watcher.
- The project list is every machine at once: `project.list` per endpoint, folded into `state/project.ts`, where a row names the endpoint it came from (`apps/client/src/project/list.ts`). The last answer of a machine is cached in localStorage, so a daemon that is not answering still lists its projects, dimmed and grouped under its own name. `openProject(endpointId, projectId)` (`apps/client/src/project/open.ts`) is the only way in and sets the active endpoint on the way; `ruimte.lastProject` holds `last` (which machine a cold boot lands on) next to `byEndpoint` (what a switch reopens). Anything drawn per row (the icon URL, a reveal in the file manager) uses that row's endpoint, never the active one.
- A loopback client needs no token; any other client pairs once (`POST /auth/pair`) and sends its session token as `?token=` on the socket (`apps/server/src/auth`). A daemon is keyed on the id it mints for itself, not on its address (`apps/client/src/state/endpoints.ts`). The client holds a socket per daemon in a `TransportPool` (`apps/client/src/transport/pool.ts`), each with its own reconnect loop; the exported `transport` is a facade over the machine that is active, `transportFor(id)` is one machine's socket, and `transport/connections.ts` builds a session and a chat client per machine (several may be alive at once) next to the one project and drawing client of the machine whose project is on screen.
- State that is about a daemon is keyed on it. `apps/client/src/state/keys.ts` is where `${endpointId}:${nodeId}` is written down, with `useEndpointId()` as the one place that says which machine the code on screen is about. The sessions, chats, browser pages, terminal screens and drawing mirrors are rows under that key; the server info, the providers and the usage numbers sit under `byEndpoint`. A switch throws nothing away any more, and forgetting a machine drops its rows. `useCanvas`, `useDocument` and `useDrawing` still hold the one project that is open.
