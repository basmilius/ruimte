...
- Components: only semantic tokens from `styles.css` (`bg-surface`, `text-text-muted`, ...). No raw colors outside that file. Tooltips are the `Tooltip` component (`src/ui/Tooltip.tsx`), never a `title` attribute. Icon buttons that belong together go in a `.btn-group`. Icons are Font Awesome Pro regular through the `Icon` component (`src/ui/Icon.tsx`), never another icon set. No fractional pixels: sizes, spacing and stroke widths are whole numbers, and a `rem` or `em` has to resolve to one.
