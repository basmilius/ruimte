...
export interface FakeCodexOptions {
    // How the note a host repeats in front of a resumed thread's first prompt starts; `pasted?` answers it back.
    resumeLead?: string;
}
...
export const fakeCodexWith =
    ...
    (io) => {
        ...
        const handleTurnStart = (params: Frame): void => {
            ...
            const noted = options.resumeLead !== undefined && raw.startsWith(options.resumeLead) && gap !== -1;
            if (noted) {
            ...
            switch (method) {
                ...
                case 'model/list':
                    out({
                        id,
                        result: {
                            data: [{ model: threadModel, inputModalities: threadModel.includes('spark') ? ['text'] : ['text', 'image'] }],
                            nextCursor: null
                        }
                    });
                    return;
