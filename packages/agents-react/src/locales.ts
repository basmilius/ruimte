...
export const AGENTS_NAMESPACES = ['agent-chat'] as const;
...
export const AGENTS_LOCALES: Record<string, () => Promise<Record<AgentsNamespace, Bundle>>> = {
    en: () => open({ 'agent-chat': () => import('./locales/en/agent-chat.json') }),
    nl: () => open({ 'agent-chat': () => import('./locales/nl/agent-chat.json') })
};
