...
describe('subagent list', () => {
    ...
    test('a child that finished its task and was given another runs again in the list, the badge and the count', () => {
        ...
        const tasks: Record<string, SubagentTask> = { a: task('done'), b: task('open') };
    ...
    test('a task whose child stopped on a limit is paused, where the row itself only knows it runs', () => {
        ...
        expect(statusWordOf(row, { ...task('open'), paused: {} })).toBe('paused');
...
describe('the time on the right of an entry', () => {
    ...
    test("a task's own record gives its times and says it was cancelled", () => {
        ...
        const task = { createdAt: given, settledAt: null, status: 'open' } as SubagentTask;
        const row = subagent('t', { origin: 'ruimte', childId: 'node-1', startedAt: noon - 1_000 });
        expect(entryTimeOf(row, task, noon)).toBe('1m 5s');
        const settled = { ...task, settledAt: noon - 30_000, status: 'cancelled' } as SubagentTask;
    ...
    test('a paused task says until when, or only that it is paused when the limit named no time', () => {
        ...
        const paused = { createdAt: noon - 65_000, settledAt: null, status: 'open', paused: { until: reset } } as SubagentTask;
        expect(entryTimeOf(row, paused, noon)).toBe(`paused until ${formatMoment(reset, noon)}`);
        expect(entryTimeOf(row, { ...paused, paused: {} }, noon)).toBe('paused');
    });
});
