...
| `--key-announces-per-minute`  | `10`        | Proven announcements per key per minute; one over it gets `rate-limited` and a closed socket, and the socket that already holds the key stays. A hello never counts, so nobody without the key can spend it.                                |
...
| `--ip-ice-per-minute`         | `30`        | `ice` questions per address per minute over every key behind it, since a key costs nothing to make; one over it gets `rate-limited` with its id.                                                                                            |
...
| `--turn-ttl-seconds`          | `3600`      | How long a credential lives, from 60 seconds to a week. A daemon asks again at two thirds of it.                                                                                                                                            |
...
valid for an hour. ICE still prefers a direct pair; the relay is only one more candidate. Nothing changes
until `--turn` is set.
...
lifetime, so the API is asked once per forty minutes with the default hour.
