...
public struct AddressBookClient: SessionAPI, Sendable {
    ...
    public init(baseURL: URL = URL(string: WireConstants.addressBookURL)!, fetch: Fetch? = nil) {
        ...
        self.fetch = fetch ?? { request in
            ...
            guard let response = response as? HTTPURLResponse else {
                throw AddressBookRequestError(code: "bad-answer", status: 0, message: "The address book returned no HTTP response.")
            }
    ...
    public func signalAccess(accessToken: String, machineID: String, key: any SessionSigner, label: String) async throws -> SignalAccess {
        let nonce = try Base64URL.randomToken(bytes: 18)
        let signature = try key.sign(SigningBytes.accessRequest(machineID: machineID, publicKey: key.publicKey, nonce: nonce))
        let statement = try await requestStatement(accessToken: accessToken, payload: AccessRequestPayload(
            machineId: machineID, clientPublicKey: key.publicKey, nonce: nonce, signature: signature
        ))
        guard statement.machineId == machineID, statement.clientPublicKey == key.publicKey, statement.nonce == nonce else {
            throw AddressBookRequestError(code: "bad-answer", status: 200, message: "Your account answered with a statement for something else.")
        }
        return SignalAccess(statement: statement, label: boundedDeviceLabel(label))
    }

    private func call<Result: Decodable>(_ method: String, path: String, token: String? = nil, body: Data? = nil) async throws -> Result {
        let (data, response) = try await response(method, path: path, token: token, body: body)
        do {
            return try JSONDecoder().decode(Result.self, from: data)
        } catch {
            throw AddressBookRequestError(code: "bad-answer", status: response.statusCode, message: "The address book answered with something this client cannot read.")
        }
    }

    private func response(_ method: String, path: String, token: String? = nil, body: Data? = nil) async throws -> (Data, HTTPURLResponse) {
        guard let url = URL(string: path, relativeTo: baseURL)?.absoluteURL, url.scheme == "https", url.user == nil, url.password == nil else {
            throw AddressBookRequestError(code: "bad-request", status: 0, message: "The address book needs an HTTPS address.")
        }
        ...
        guard (200..<300).contains(response.statusCode) else {
            if let failure = try? JSONDecoder().decode(AddressBookError.self, from: data) {
                throw AddressBookRequestError(code: failure.error.code.rawValue, status: response.statusCode, message: failure.error.message)
            }
            throw AddressBookRequestError(code: "bad-answer", status: response.statusCode, message: "The address book answered \(response.statusCode).")
        }
        return (data, response)
    }
}
