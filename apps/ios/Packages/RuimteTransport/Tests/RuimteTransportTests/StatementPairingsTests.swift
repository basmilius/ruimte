...
final class StatementPairingsTests: XCTestCase {
    @MainActor func testAcceptedStatementSurvivesRestartAndReconnectsWithoutAddressBook() async throws {
        ...
        let identity = PairingIdentity(machineID: "machine", machineKey: DeviceKey().publicKey, clientKey: DeviceKey().publicKey)
        var requests = 0
        var opens = 0
        let events = LinkEvents(opened: { opens += 1 }, message: { _ in }, closed: { _ in })
        let first = try XCTUnwrap(StatementPairings(store: store).open(identity: identity, requestAccess: {
            requests += 1
            return .object([:])
        ...
        let restarted = StatementPairings(store: UserDefaultsPairingStore(defaults: try XCTUnwrap(UserDefaults(suiteName: domain))))
        let second = try XCTUnwrap(restarted.open(identity: identity, requestAccess: {
            requests += 1
    ...
    @MainActor func testRefusalDoesNotSettleOrRestoreStatementAccess() throws {
        ...
        let first = try XCTUnwrap(pairings.open(identity: identity, requestAccess: { .object([:]) }, events: events,
        ...
        first.events.opened()
        XCTAssertFalse(store.contains(identity))
        let accepted = try XCTUnwrap(pairings.open(identity: identity, requestAccess: { .object([:]) }, events: events,
                                                 makeLink: { PairingTestLink(access: $0, events: $1) }) as? PairingTestLink)
        XCTAssertNotNil(accepted.access)
        accepted.events.opened()
        let second = try XCTUnwrap(pairings.open(identity: identity, requestAccess: { .object([:]) }, events: events,
                                               makeLink: { PairingTestLink(access: $0, events: $1) }) as? PairingTestLink)
        XCTAssertNil(second.access)
        second.events.closed(TransportFailure.invalid("not-paired"))
        let retry = try XCTUnwrap(pairings.open(identity: identity, requestAccess: { .object([:]) }, events: events,
                                              makeLink: { PairingTestLink(access: $0, events: $1) }) as? PairingTestLink)
    ...
    @MainActor func testSettlementIsBoundToMachineAndBothPinnedKeys() {
        ...
        let changed = [
            ...
            PairingIdentity(machineID: identity.machineID, machineKey: DeviceKey().publicKey, clientKey: identity.clientKey),
            PairingIdentity(machineID: identity.machineID, machineKey: identity.machineKey, clientKey: DeviceKey().publicKey)
        ]
        for identity in changed {
            _ = pairings.open(identity: identity, requestAccess: { .object([:]) }, events: LinkEvents(opened: {}, message: { _ in }, closed: { _ in }), makeLink: { access, events in
