...
const usage = (): void => {
    notify('thread/tokenUsage/updated', {
        ...
        turnId,
        tokenUsage: {
            total: { totalTokens: 25090, inputTokens: 25085, cachedInputTokens: 12928, cacheWriteInputTokens: 0, outputTokens: 5, reasoningOutputTokens: 0 },
...
const turnCompleted = (status: 'completed' | 'interrupted' | 'failed', error: string | null = null): void => {
    ...
    notify('turn/completed', {
        threadId,
        turn: { id: turnId, items: [], itemsView: 'summary', status, error: error ? { message: error, codexErrorInfo: null, additionalDetails: null } : null }
    });
    turnId = '';
};
...
const handleApprovalResponse = (result: Frame): void => {
...
const handleQuestionResponse = (result: Frame): void => {
...
const handleRequest = (id: unknown, method: string, params: Frame): void => {
    switch (method) {
        case 'initialize':
            out({ id, result: { userAgent: 'fake-codex/0.0.0', codexHome: process.cwd(), platformFamily: 'unix', platformOs: 'test' } });
            return;
        case 'thread/start':
        case 'thread/resume': {
...
for await (const chunk of Bun.stdin.stream()) {
    ...
    while (newline >= 0) {
        ...
        } else if (frame.id !== undefined && frame.result !== undefined) {
            const result = frame.result as Frame;
            // Read through a function: the flow analysis otherwise narrows these to never inside the loop.
            const waiting = (): { approval: PendingApproval | null; question: { rpcId: number } | null } => ({
                approval: pendingApproval,
                question: pendingQuestion
            });
            const { approval, question } = waiting();
            if (approval && approval.rpcId === frame.id) {
                handleApprovalResponse(result);
