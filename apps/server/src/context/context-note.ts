...
export const verbsNote = ({ depth, standalone = false }: { depth: number; standalone?: boolean }): string => {
    if (standalone) {
        return [
            'Ruimte: you are in a standalone AI-chat view, not a node on a canvas.',
            "Use your CLI's own subagents for delegated work within this chat when the task calls for it. Preserve any model choice the person requested.",
            '`ruimte-context` is a shell command for reading linked context and managing project views and canvas nodes. `ruimte-context help` lists its verbs and nouns.',
            'Its `agent` and `team` commands create persistent nodes on a canvas; use those when the person asks for canvas agents, and name the target with --view. They are not the subagents of this chat.',
 ...
 */
export const chatPrompt = ({ sources, depth, standalone }: { sources: ContextSource[]; depth: number; standalone?: boolean }): string => {
    const note = verbsNote({ depth, standalone });
