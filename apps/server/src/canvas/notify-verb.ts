    ...
    async run({ positionals: [id], flags }, call) {
        ...
        const lines = (): string[] =>
            orNote(
                ...
                'No line runs from you into a terminal or a chat; ruimte-context link --to <id> draws one'
            );
        if (!target) {
        ...
        if (!reachable.some((node) => node.id === id)) {
            throw new VerbRefusal(
                'not-linked',
                `No line runs from you into ${id}, so it cannot be notified; a message travels the way a read does, and only that way`,
