...
const ARRANGE_DETAIL: readonly string[] = [
    ...
    'ids\tOnly ids, never titles; ruimte-context nodes lists the nodes of a canvas with theirs and their sizes'
];

export const arrangeVerb = defineVerb({
    name: 'arrange',
    ...
    positionals: z.tuple([], { error: 'arrange takes no arguments, only flags; the nodes go in --nodes' }),
    flags: z.object({
        ...
        return call.host.mutate(place.projectId, (content) => {
            ...
            if (carrier) {
                throw new VerbRefusal(
                    'not-arrangeable',
                    `${carrier.id} is a group and carries whatever stands inside it; arrange moves the nodes it is given, so name those instead`
                );
            }
