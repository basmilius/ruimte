...
import { AGENT_HOLD_IDLE_MS, DeviceDriver, type DeviceGate } from './agent-driver.ts';
...
beforeEach(async () => {
    ...
    driver = new DeviceDriver({
        ...
        gate: { refusal: (nodeId) => gate.refusal(nodeId) }
    });
});
...
describe('DeviceDriver', () => {
    test('writes a shot under the home folder and counts later coordinates in its pixels', async () => {
        ...
        const shot = await driver.shot(SIMULATOR, 'phone-node');
        ...
        await driver.tap('phone-node', 'chat-1', SIMULATOR, { x: 250, y: 500 });
    ...
    test('refuses a coordinate before any shot, and one outside the last shot', async () => {
        await expect(driver.tap('phone-node', 'chat-1', SIMULATOR, { x: 1, y: 1 })).rejects.toMatchObject({ code: 'no-shot' });
        await driver.shot(SIMULATOR, 'phone-node');
        await expect(driver.tap('phone-node', 'chat-1', SIMULATOR, { x: 1206, y: 10 })).rejects.toMatchObject({ code: 'outside-shot' });
    ...
    test('a swipe presses, moves a frame at a time over its duration and lets go at the end', async () => {
        ...
        await driver.shot(SIMULATOR, 'phone-node');
        await driver.swipe('phone-node', 'chat-1', SIMULATOR, { x: 50, y: 80 }, { x: 50, y: 20 }, 64);
    ...
    test('holds a session without any client, which a client opening the device shares', async () => {
        await driver.shot(SIMULATOR, 'phone-node');
        await driver.button('phone-node', 'chat-1', SIMULATOR, 'home');
        ...
        await driver.tap('phone-node', 'chat-1', SIMULATOR, { x: 10, y: 10 });
        expect(backend.source.starts).toBe(1);
    ...
    test('passes every gesture and launch through the gate, and nothing reaches the device past a refusal', async () => {
        await driver.shot(SIMULATOR, 'phone-node');
        gate = { refusal: (nodeId) => (nodeId === 'phone-node' ? { code: 'paused', message: 'The person paused this device' } : null) };
        await expect(driver.tap('phone-node', 'chat-1', SIMULATOR, { x: 10, y: 10 })).rejects.toMatchObject({ code: 'paused' });
        await expect(driver.launch('phone-node', SIMULATOR, 'com.example.app')).rejects.toMatchObject({ code: 'paused' });
        expect(backend.source.starts).toBe(0);
    ...
    test('refuses input on a device that takes none, and says what works', async () => {
        ...
        await driver.shot(readOnly, 'phone-node');
        await expect(driver.tap('phone-node', 'chat-1', readOnly, { x: 10, y: 10 })).rejects.toMatchObject({ code: 'device-input-unavailable' });
