...
export const encryptPush = (routing: PushRouting, publicKey: string, content: PushAlertContent, sign: (message: string) => string): PushEnvelope => {
    ...
    const fitted = PushAlertContentSchema.parse(content);
    ...
    while (bytes.length > 2200 && fitted.body.length > 0) {
