...
The container holds the native daemon, `ruimte-context`, a shell, git and ripgrep. No Claude Code, no Codex, so chat nodes
and agent status are out of scope here.

## Run

```sh
bun run --cwd apps/server-rust docker:up        # build and start on 127.0.0.1:4310
bun run --cwd apps/server-rust docker:pair      # a pairing URL to paste into the app
bun run --cwd apps/server-rust docker:test      # the test suite, against a container of its own
bun run --cwd apps/server-rust docker:down      # stop and remove it, its state kept
bun run --cwd apps/server-rust docker:reset     # stop it and throw its state away
```
...
The image compiles `ruimte-server` and `ruimte-context` in a pinned Rust builder, then copies only
the two executables and required system tools into Debian slim. The persistent daemon uses a release
build. The disposable test daemon uses a debug build so its test statement key remains available.
...
a machine that has been worked on, so `docker:test` runs it against `daemon-test`, a debug build of
the same native source on 127.0.0.1:4320 under the name `ruimte-remote-test`. It carries no volumes,
...
`apps/server-rust/tests/remote-daemon.integration.test.ts` pairs, opens a socket and walks the wire: `/health`,
...
`ruimte-context` is installed in `/usr/local/bin`, which is also the path the daemon gives agent
