...
pub struct Config {
    ...
    /// Optional character width provider used instead of `unicode-width`.
    pub character_width: Option<CharacterWidth>,

    /// Clear an existing soft-wrap boundary when LF crosses it.
    pub clear_wrapline_on_linefeed: bool,

    /// Repair wide-character pairs after insert/delete operations.
    pub repair_wide_cells_after_edit: bool,

    /// Match xterm's DECSC behavior by excluding pending wrap from the saved cursor.
    pub clear_wrap_on_save_cursor: bool,

    /// Keep a logical soft-wrap boundary when editing cells in its source row.
    pub preserve_wrapline_after_edit: bool,
}

/// Lets an embedding terminal match the Unicode width version used by its renderer.
#[derive(Clone, Copy)]
pub struct CharacterWidth(fn(char) -> usize);

impl CharacterWidth {
    pub const fn new(callback: fn(char) -> usize) -> Self {
        Self(callback)
    }

    fn width(self, character: char) -> usize {
        (self.0)(character)
    }
}

impl std::fmt::Debug for CharacterWidth {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.write_str("CharacterWidth(..)")
    }
}
...
fn repair_wide_cell_pairs(row: &mut [Cell], background: Color) {
    for index in 0..row.len() {
        let valid = if row[index].flags.contains(Flags::WIDE_CHAR) {
            row.get(index + 1)
                .is_some_and(|cell| cell.flags.contains(Flags::WIDE_CHAR_SPACER))
        } else if row[index].flags.contains(Flags::WIDE_CHAR_SPACER) {
            index > 0 && row[index - 1].flags.contains(Flags::WIDE_CHAR)
        } else {
            true
        };
        if !valid {
            row[index] = background.into();
        }
    }
}
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            clear_wrapline_on_linefeed: false,
            repair_wide_cells_after_edit: false,
            clear_wrap_on_save_cursor: false,
            preserve_wrapline_after_edit: false,
        }
    }
}
...
impl<T> Term<T> {
    ...
    /// Access to the currently inactive terminal grid.
    ///
    /// Ruimte serializes both buffers while the alternate screen is active. A read-only accessor
    /// avoids mutating through `swap_alt`, which clears the alternate grid when it is re-entered.
    pub fn inactive_grid(&self) -> &Grid<Cell> {
        &self.inactive_grid
    }

    /// Access to the active vertical scroll region.
    ///
    /// Ruimte restores terminal snapshots through ANSI and needs the region to preserve origin
    /// mode behavior after reconnecting. Returning a copy keeps terminal state immutable.
    ...
    fn write_at_cursor(&mut self, c: char) {
        ...
        {
            flags.insert(Flags::XTERM_EXPLICIT_SPACE);
        }
...
impl<T: EventListener> Handler for Term<T> {
    ...
    fn input(&mut self, c: char) {
        ...
        let width = match self.config.character_width {
            Some(character_width) => character_width.width(c),
            None => match c.width() {
                Some(width) => width,
                None => return,
            },
    ...
    fn insert_blank(&mut self, count: usize) {
        ...
        let repair_wide_cells = self.config.repair_wide_cells_after_edit;
        let preserve_wrapline = self.config.preserve_wrapline_after_edit
            && self.grid[cursor.point.line][self.grid.last_column()]
                .flags
                .contains(Flags::WRAPLINE);
        ...
        if repair_wide_cells {
            repair_wide_cell_pairs(row, bg);
        }
        if preserve_wrapline {
            row.last_mut().unwrap().flags.insert(Flags::WRAPLINE);
        }
    }
    ...
    fn linefeed(&mut self) {
        trace!("Linefeed");
        if self.config.clear_wrapline_on_linefeed {
            let line = self.grid.cursor.point.line;
            let last_column = self.grid.last_column();
            let flags = &mut self.grid[line][last_column].flags;
    ...
    fn erase_chars(&mut self, count: usize) {
        ...
        let preserve_wrapline = self.config.preserve_wrapline_after_edit
            && self.grid[line][self.grid.last_column()]
                .flags
                .contains(Flags::WRAPLINE);
        let row = &mut self.grid[line];
    ...
    fn save_cursor_position(&mut self) {
        ...
        self.grid.saved_cursor = self.grid.cursor.clone();
        if self.config.clear_wrap_on_save_cursor {
            self.grid.saved_cursor.input_needs_wrap = false;
        }
    }
    ...
    fn clear_line(&mut self, mode: ansi::LineClearMode) {
        ...
        let preserve_wrapline = self.config.preserve_wrapline_after_edit
            && self.grid[point.line][self.grid.last_column()]
                .flags
                .contains(Flags::WRAPLINE);
