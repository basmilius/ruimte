...
import { DirectClient, type DirectCredential } from './support/direct-client.ts';
import {
    BYTES_CHUNK_MAX,
    clientAuthMessage,
...
const pair = async (publicKey?: string): Promise<PairResult> => {
    const printed = await inContainer(['/usr/local/bin/ruimte-server', 'pair', '--port', String(PORT)]);
...
describe.skipIf(!ENABLED)('the daemon in the Linux container', () => {
    ...
    beforeAll(async () => {
        ...
        await waitUntil(`a daemon on ${BASE_URL}; start one with \`bun run --cwd apps/server-rust docker:test\``, answers, 5_000);
        paired = await pair();
    ...
    test('the daemon keeps the endpoint identity it serves in its home', async () => {
        const info = await client.request<EndpointInfo>('endpoint.info', {});
...
describe.skipIf(!ENABLED)('two daemons at the same time', () => {
    ...
    beforeAll(async () => {
        ...
        here = await RemoteClient.connect((await readFile(`${home}/local.key`, 'utf8')).trim(), LOCAL_PORT);
...
describe.skipIf(!ENABLED)('one id on two machines', () => {
    ...
    test('a note linked on a canvas of the container is read back inside that container', async () => {
        ...
        await there.request('session.write', {
            sessionId,
            data: '/usr/local/bin/ruimte-context && /usr/local/bin/ruimte-context read note-1\n'
        });
        let said = '';
...
describe.skipIf(!ENABLED)('two projects side by side', () => {
    ...
    test('the bytes a viewer draws come from the daemon of its own workspace', async () => {
        ...
        const localSecret = encodeURIComponent((await readFile(`${home}/local.key`, 'utf8')).trim() ?? '');
