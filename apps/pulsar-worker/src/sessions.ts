import type { ProviderId, SessionResult } from '@ruimte/pulsar';
...
export const createSession = async (db: D1Database, account: AccountRow, label: string | null): Promise<SessionResult> => {
    const now = Date.now();
    ...
    await db
        .prepare(
            'INSERT INTO session (id, account_id, label, access_hash, access_expires_at, refresh_hash, expires_at, created_at) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8)'
        )
        .bind(crypto.randomUUID(), account.id, label, await sha256(accessToken), accessExpiresAt, await sha256(refreshToken), expiresAt, now)
        .run();
 ...
 */
export const rotateSession = async (db: D1Database, refreshToken: string): Promise<SessionResult | null> => {
    const now = Date.now();
    ...
    const row = await db
        .prepare(
            `UPDATE session SET access_hash = ?1, access_expires_at = ?2, refresh_hash = ?3, previous_refresh_hash = refresh_hash
             WHERE refresh_hash = ?4 AND revoked_at IS NULL AND expires_at > ?5
        ...
        .bind(await sha256(accessToken), accessExpiresAt, await sha256(nextRefreshToken), hash, now)
        .first<{ expires_at: number; account_id: string; provider: ProviderId; login: string | null }>();
    if (!row) {
        await db.prepare('UPDATE session SET revoked_at = ?1 WHERE previous_refresh_hash = ?2 AND revoked_at IS NULL').bind(now, hash).run();
        return null;
    }
    return {
        accessToken,
