...
describe('how a state looks', () => {
    ...
    test("a task's open state is the running one everything else draws", () => {
        expect(taskStatusWord('open')).toBe('running');
        expect(taskStatusWord('cancelled')).toBe('cancelled');
    });
});
