...
import { FileDiff, FileWarning, GitBranch, GitCompare, LoaderCircle, WrapText } from 'lucide-react';
...
export function DiffFile({ tabKey, path, name, view }: { tabKey: string; path: string; name: string; view: FileTabView }) {
    ...
    const asked = `${view.cwd}\u0000${relative}\u0000${view.scope}\u0000${String(view.staged)}\u0000${nonce}`;
    ...
    useEffect(() => {
        let alive = true;
        transport
            .request('git.diff', { cwd: view.cwd, path: relative, scope: view.scope, staged: view.staged })
            .then((diff) => {
                if (alive) {
    ...
    }, [asked, relative, view.cwd, view.scope, view.staged]);
    ...
    return (
        <FileActionsContext.Provider value={actions}>
            <div className="flex min-h-0 min-w-0 grow flex-col">
                ...
                <DiffBody state={state} wrap={wrap} relative={relative} />
            </div>
        </FileActionsContext.Provider>
    );
}

function DiffBody({ state, wrap, relative }: { state: DiffState; wrap: boolean; relative: string }) {
    ...
    return (
        <div className="file-diff min-h-0 grow overflow-auto">
            <Suspense fallback={<div className="px-3 py-2 text-xs text-text-faint">Loading diff</div>}>
                <UnifiedDiff change={{ path: relative, kind: 'update', diff: state.diff.diff }} overflow={wrap ? 'wrap' : 'scroll'} />
            </Suspense>
        </div>
    );
}
