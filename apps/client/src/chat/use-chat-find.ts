...
/*
 * Find in a thread. The hits come from the data (`chat/search.ts`), so a folded turn and a closed
...
export const useTimelineFind = (options: TimelineFindOptions): TimelineFind => {
    ...
    return {
        ...
        bar: createElement(FindBar, {
            find: chatFind.find,
            total: chatFind.total,
            current: chatFind.current,
            invalid: chatFind.invalid,
            onStep: chatFind.step
        }),
        reveal: chatFind.reveal,
