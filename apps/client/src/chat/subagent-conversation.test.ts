...
import type { ChatItem, ChatSubagentPayload, ChatSubagentResult, EventMap, EventType, RequestMap, RequestType } from '@ruimte/contracts';
...
class FakeMachine implements Transport {
    ...
    request<T extends RequestType>(type: T, payload: RequestMap[T]['payload']): Promise<RequestMap[T]['result']> {
        if (type !== 'chat.subagent') {
            return Promise.reject(new TransportError('unknown-request', type));
        }
        ...
        return Promise.resolve(result as RequestMap[T]['result']);
    }
...
describe('SubagentConversation', () => {
    ...
    test('a machine from before the panel says so, and a history that expired starts over from the newest end', async () => {
        ...
        old.fail = new TransportError('unknown-request', 'Unknown request type: chat.subagent');
        ...
        machine.failNext = new TransportError('history-expired', 'The conversation changed. Reload its history.');
        await conversation.loadEarlier();
