...
const DECISION: Record<Exclude<ChatApprovalItem['decision'], 'pending'>, { label: string; icon: React.ReactNode }> = {
    allow: { label: 'Allowed', icon: <Check size={12} /> },
    'allow-always': { label: 'Always allowed', icon: <Check size={12} /> },
    deny: { label: 'Declined', icon: <X size={12} /> },
    cancelled: { label: 'No longer needed', icon: <X size={12} /> }
};
        ...
        <div className="pb-2">
            {item.questions.map((question) => (
                <div key={question.id} className="flex items-start gap-2 px-1 py-0.5 text-[12px] text-text-faint">
                    <MessageCircleQuestion size={13} className="mt-0.5 shrink-0" />
