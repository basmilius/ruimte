...
- A failure reaches a person only through `failure.ts`. Each `DictationError` code is a key under `voice:dictation.errors`, and a person never sees a raw code or a Chromium message.
