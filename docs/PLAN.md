...
open issues (#11, #12, #13), and three studies of nodeterm and T3 Code (behavior only, never code).
...
a folder icon for the files panel. Then: a collapsible sidebar the way T3 Code has one, the Files and
...
- Files: `fs.list` (files and directories with type and size) and a bounded `fs.read` with a binary
  check; a tree with filter over `fs.search`, drag a file onto a chat node as a mention, a read-only
  viewer with shiki, markdown and images. No file manager.
...
Several days. This is where Ruimte earns its name against nodeterm.
...
panes. From T3 Code: the PR client, Clerk cloud and relay, SSH and WSL environments, MCP browser
automation, cookie and theme import, usage scanning, the mobile app. Neither has fork, retry or
edit-and-resend worth copying; T3 Code has no OS notifications or dock badge at all.
