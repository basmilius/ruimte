...
iets." T3 Code does not do that either, and the reason is 1.1: their browser is a **pushed step**
...
- **Machines.** With one machine known, browsing opens on that machine's folders and the machines
  are never asked about, which is what T3 Code does with one environment. With more than one it
...
T3 Code's palette holds a stack of views (`viewStack`, `CommandPalette.tsx:707`). A view is an icon,
...
- Emptying the field does **not** leave browse mode any more. It cannot: an empty field is what the
  machines step has, and it is what Backspace needs something to step back from. T3 Code pops the
...
what theirs always is. This is a deliberate deviation: T3 Code shows nothing at all while browsing
...
replaces them. `apps/client/src/App.tsx` mounts nothing new. No Cmd+O: T3 Code has no chord of its own for this and the palette is one
