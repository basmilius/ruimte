...
- A path that does not exist is offered for creation inline: the button
  reads "Create and open", it creates the whole missing chain, and there is no separate "New folder"
  action. Git repositories get no list, no scan and no mark.
...
- The palette flow, for the record, is the thing this proposal moves away from: its picker is a wizard
...
**Git repositories: nothing.** Decided with Bas. The folder browser
...
(1.1). So "none" means: no repository list, no repository scan, and **no `hasGit` mark
...
Ruimte does keep is the `hasCanvas` mark that `fs.browse` already returns
...
If a repository list is ever wanted, there is a better source than a walk: the
...
The picker should say it before Enter, and then offer to create it (1.4).
...
**How the picker decides a folder is not there.** Two cases, from two tests (1.4)
...
edge case that comes with it: typing `~/pro` while `~/projects` exists reads as "not there", so the
...
**What creating does.** No new request, a flag on the open: `ProjectOpenPayload`
...
flag; every other caller of `project.open` keeps today's behavior and leaves it unset
...
- Something is there and it is not a directory. The `mkdir` must not run: `isDirectory` already
  ...
  than in a toast. A toast is easy to miss; inline is better and costs nothing here.
...
**No separate "New folder" button.** None is needed (1.4). The native
...
`hasGit` on an entry was in an earlier draft of this document and is **out**, per 3.4: the browser does
...
- `ProjectOpenPayload.createFolder: z.boolean().optional()`. Only the picker sends it, and only when
  the folder is not there (3.8). This is a flag on the command that
...
- **Creating a folder nobody meant to create.** The rule of 3.8 says `~/pro` does not exist while
  `~/projects` sits in the list, so a fast Enter makes `~/pro`. That is accepted. The guard
...
Nothing here is open. Two of these are Bas's, answered as "keep it the palette way"; the rest are
settled by this document and each one is a small edit if he wants it the other way.

1. **Git repositories in the picker: none.** Bas. The folder browser has no
...
2. **A path that does not exist: offer to create it, inline.** Bas. The button
...
3. **The picker opens a project.** It does not hand a path back to a caller. That is the only thing
   a folder is asked for today, and it could go further still by dropping you into a fresh thread
...
4. **The machine chip is a label, not a switcher.** The palette flow picks the machine as an explicit first
...
5. **Hidden folders get a toggle**, which is the one place this deliberately does not follow the palette
   conventions. Elsewhere a dot folder appears only once the typed segment starts with a dot, a rule
