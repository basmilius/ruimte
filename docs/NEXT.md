...
2. **The daemon as a background service** is built (`apps/desktop/src/service`, `packages/service`)
   and waits for a packaged release to be tried, updates included (`apps/server/src/service`). The
   ruimte.app landing page comes later and gets an issue when it starts.
3. **A third chat provider** (Gemini, Copilot or opencode) as the proof that the backend seam
   ...
   Gemini and Copilot are a day per CLI on top. Two things come before it. The iPhone app with open
   enums (`AgentKind`, `RuntimeMode` and `AgentStatus` are `x-open-enum` in `schemas.json`, since
...
4. **Approvals for the other terminal CLIs.** Codex's `PermissionRequest` hook is installed and
   takes the same path as Claude Code's, but whether Codex honors the decision it gets back is
   unproven. Gemini and Copilot have no hook events yet and wait on their hooks in 3.
5. **Terminal basics**, about two days. Search on Cmd+F (the shared find bar in
   ...
   that is not this.
6. **Canvas ergonomics**, about two days, all client state. Directional focus between nodes and
...
7. **Chat depth**, several days. A proposed plan card with "Implement" and "Implement in a new
   node" (a chat node beside it with a context edge, so the new agent reads the plan through
   `ruimte-context`), review comments from a diff into the prompt, and approval choices in the
...
8. **Settings and keyboard**: one binding table with `when` contexts, read by the handlers and the
...
9. **Per-project settings** in `.ruimte/settings.json`: the file, `project.settings` and its first
   field (`worktrees.share`) are there; the terminal agent mode is next, then clone a repository as
   a project. From worktrees: the iOS app shows none of it beyond forking into one, and a merge into
   a branch checked out nowhere is refused (`target-not-checked-out`) rather than offered as a
   ref-only merge.
10. **Around the editor.** A text file opens in the editor (`packages/editor`) in the files panel
...
11. **A test floor**: a dev-only 30-node palette command and whatever it finds; a DOM setup for
    `bun test` with first specs for the composer and the canvas wiring.
12. **Usage v2**: price and plan overrides and an export. The Days table is there, and so is a USD
    and EUR switch on the usage page; any other currency or a setting for it is not.
13. **More than one window**: a window shows a start screen or one project, and two projects side by
...
14. **Smaller ones**: a color or an arrowhead per plain line; a note's title as the first heading
    ...
    startup gains.
15. **Remote access leftovers**: a production broker on a server of its own, after which the test
    ...
    unverified.
16. **Devices** (`docs/reports/2026-09-13-devices.html`): the iOS Simulator, physical iPhones and
    iPads, and Android emulators and phones (on `adb` and a pinned scrcpy server) run as a panel, a
    view and a node, on the stream hub a browser page uses. An agent operates a device over a line
    with `ruimte-context device`, and a person can pause it or take over. What is left is the
    acceptance on a real device: that device visible over the network again, and stream and control
    together in all three places. Unmeasured: LAN bandwidth, two viewers at once, rotation, click to
    paint, and a helper that crashes under an open view. The one local measurement was 17.4 fps
    against a goal of 30. Typing on a physical iPhone is not there (the simulator and Android have
    it). The iPhone app knows a device from the generated schema and draws none of it. Out on
    purpose: installing an app and logs.
17. **Plugins** (`docs/reports/2026-09-14-plugins.html`, decisions pending): internal registries
...
18. **Several accounts per CLI** (`docs/reports/2026-09-10-accounts.html`, not decided): one config
    ...
    in `project.json`.
19. **Orchestration leftovers** (`docs/reports/2026-09-24-orchestration-upstream.html`). Built and
