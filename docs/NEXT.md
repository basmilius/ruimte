...
9. **The action registry** (`packages/actions`): one typed layer under everything a person can do,
   ...
   execution. The registry and its catalog stand, with the sidebar, the view dialogs, the voice tools
   and the `view`, `node` and `link` verbs on it (the daemon's handlers in `apps/server/src/actions`).
   The keyboard handlers, the command palette and the other verbs (agent, team, worktree, notify,
   task, plan, diagram, browser) still carry their own path; moving them over is the rest of it, and
   the binding table in 10 hangs on it.
