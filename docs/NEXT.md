...
9. **The action registry** (`packages/actions`): one typed layer under everything a person can do,
   ...
   composer, the prompt cards, the fork dialog, the plan panel, the voice tools (generated from the
   catalog) and every `ruimte-context` verb but `help`, `list` and `read` on it (the daemon's
   handlers in `apps/server/src/actions`). Which actor may run what is decided per action; the
   report's "Wie mag wat" has the tables, Git and then chats and terminals. `agent.start`
   ...
   cancelled with `git.cancel`), a person's agent start on the registry, the domains after chats
   and terminals in "Wie mag wat", Restart and Resume under an ended shell (they rebuild the node's
   xterm), and a file dropped on a cell of the grid, which no action can place since none names a
   cell. The binding table in 10 hangs on it.
