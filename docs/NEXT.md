...
9. **The action registry** (`packages/actions`): one typed layer under everything a person can do,
   ...
   measures its tool choice, codes, target errors and latency on this machine only and writes each
   request and a session summary to the developer console as `[voice]` debug lines
   (`apps/client/src/voice/diagnostics.ts`). The binding table in 10 hangs on it.
