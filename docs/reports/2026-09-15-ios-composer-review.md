...
## Large chat history breaks the receive limit

The stored App: Codex conversation (`chat-iz4bhwwa`) contained 2,871 items and 17,731,272 UTF-16 units when inspected. Most of that size was tool output. The deployed daemon returns the complete history in `chat.attach`. The mobile transport rejected any authenticated response above 16,777,216 units, closed the shared machine connection and showed its generic unreadable-frame error. A normal growing conversation can therefore trigger a transport failure without containing an unsupported message type.

The mobile receiver now accepts authenticated frames up to 64 Mi UTF-16 units. The pre-authentication ceiling stays at 4,096 units. Oversized responses and fragments missing their wire marker have distinct errors. Opt-in diagnostics report the size of large completed frames without their content. This accommodates the deployed daemon's full snapshots; it is a bounded compatibility fix, not paginated history. A snapshot exceeding the new ceiling will still be rejected. Loading history in pages remains necessary for sustained growth and lower transfer and memory costs.
