...
- **Phase 6b, the chat the way T3 Code does it** (studied, then written from scratch):
  ...
  localStorage. The model picker has T3's shape: the provider's mark plus the short model name
...
- **Phase 7, Electron shell and browser node**: `apps/desktop` is a small main process
  ...
  which is how that band gets measured rather than guessed. The title bar follows T3 Code:
